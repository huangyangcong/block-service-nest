...
export class KafkaService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    await this.connect();
    SUBSCRIBER_FN_REF_MAP.forEach((functionRef: Function, topic: string) => {
      this.bindAllTopicToConsumer(functionRef, topic);
    });

    SUBSCRIBER_FIXED_FN_REF_MAP.forEach((functionRef: Function, topic: string) => {
  ...
  async bindAllTopicToConsumer(callback: Function, _topic: String) {
  ...
  async bindAllTopicToFixedConsumer(callback: Function, _topic: String) {
